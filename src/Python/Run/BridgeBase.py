...
class BridgeBase:
    def __init__(self, bridge):
        ...
        if self.module.split('-')[0] == 'prs' and self.module.split('-')[-1] != 'single': 
            ...
            else:                                    self.run_eval, self.run_quantify      = self.prior_eval, self.prior_quantify 
    ...
    def prior_eval(self, name = 'eval'): 
        ...
        X.extend(['--beta.stem',self.io.paths['eval']+'/'+self.args.popname+'_eval','--by.chr.sumstats',self.args.sumstats_suffix,'--ranking','f.stat']) 
    ...
    def run_prior(self, name = 'prior'): 
        ...
        X.extend(['--precision','TRUE','--param.file',opt_params,'--by.chr.sumstats',self.args.sumstats_suffix,'--S','1','--lambda','1','--help','FALSE']) 
        X.extend(self.e_stat) 
