...
class SumStats: 
    def __init__(self, pop): 
        ...
        self.pop, self.mps, self.progress, self.fields, self.debug_level, self.savepath = pop, pop.mps, pop.progress, pop.field_key, pop.args.debug_level, pop.paths['save']   
    ...
    def load_rs_key(self, genoPheno, PP): 
        ...
        if PP: self.snp_file, self.thin_snps = PP.sumstats.snp_file, PP.sumstats.thin_snps         
    ...
    def find_sumstats_pairs(self): 
        ...
        if len(file_cands) == 0: 
            self.mps.error('InputError: Invalid Sumstats Prefix/Suffix pair, no files match\n    Prefix: '+self.source_prefix+'\n    Suffix: '+self.source_suffix) 
        if len(file_cands) == 1: 
        ...
        if self.source_suffix: 
            ...
            self.progress.start_warning('Prefix/Suffix For Pop '+self.pop.name+' Does Not Produce Unique Numerical Chromsomes','Attempting To Infer Correct Prefix/Suffix') 
        self.progress.start_warning('Sumstats Suffix Not Supplied for Pop '+self.pop.name,'Attempting to Infer Correct Prefix/Suffix')
        ...
        try: 
            ...
            if len(list(set(chr_ints))) == len(file_cands) and len(file_cands) < 25: 
                self.progress.end_warning('.......Found! (Using Valid Prefix/Suffix Pair: '+new_prefix+'/'+new_suffix+')')      
                return [[cr,prefix_path+'/'+cf]  for cr,cf in zip(chr_ints, file_cands)] 
        except: pass 
        self.progress.end_warning('.......Failed!') 
    ...
    def load_sumstats_files(self): 
        ...
        if len(sumstats_pairs) == 1: 
            if self.debug_level < 1: self.mps.error('BridgeSumstatsError: Sumstats Files Not Split by Chromosome, Please Increase Debug Level (--debug_level 2) Or Split Sumstats Files by Chromosome')           
            self.TESTS['INFER_CHR'] = True 
            self.progress.start_warning('Sumstats files for pop '+self.pop.name+' are not separated by chromosome','Splitting Sumstats in '+new_path) 
            p_handle = self.start_sumstats_file(sumstats_pairs[0][1]) 
            self.process_sumstats_file(p_handle) 
            p_handle.close() 
            self.progress.end_warning('Complete') 
        else: 
            if self.debug_level > 0: self.progress.start_warning('Verifying Sumstats File For Pop '+self.pop.name,'New Sumstats in '+new_path,WTYPE='BridgeDebugger:') 
    ...
    def split_sumstats_file2(self, p_handle): 
    ...
    def process_sumstats_file(self, p_handle): 
        for li,line in enumerate(p_handle): 
            ...
            if cInvalid == cTotal: self.mps.fail(['Sumstats File Error(s), No Matching Ref/Alt Bases']) 
            elif cInvalid > cTotal/2.0: self.mps.fail(['Sumstats File Error(s), Majority MisMatching Ref/Alt Bases - Please Check Builds']) 
        p_handle.close() 
