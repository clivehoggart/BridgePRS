...
library(doMC)
library("optparse")
#source('~/BridgePRS1/src/functions.R')
...
option_list = list(
    ...
    make_option(c("--models1"), type="character", default=NULL,help="", metavar="character"),
    make_option(c("--models2"), type="character", default=NULL,help="", metavar="character"),
    make_option(c("--eval1"), type="character", default=NULL,help="", metavar="character"),
    make_option(c("--eval2"), type="character", default=NULL,help="", metavar="character"),
    make_option(c("--outfile"), type="character", default=NULL,help="", metavar="character"),
...
p2Test  <-   paste0(opt$pred2,'_all_preds_test.dat')
p2Valid <-   paste0(opt$pred2,'_all_preds_valid.dat')
if( is.null(opt$eval1) ){opt$eval1 <- opt$models1}
if( is.null(opt$eval2) ){opt$eval2 <- opt$models2}
...
if( !is.null(opt$pred2) ){
    pred2 <- fread(p2Test, data.table=FALSE)
    if( opt$ids.col ){ pred2 <- pred2[,-1] }
    X <- as.matrix(cbind( pred1, pred2 ))
    fit.ridge <- cv.glmnet( y=target[,opt$pheno.name], x=X, family="gaussian",alpha=0, parallel=TRUE, nfolds=nfolds, grouped=FALSE )
    fit.ridge2 <- cv.glmnet( y=target[,opt$pheno.name], x=as.matrix(pred2),family="gaussian",alpha=0, parallel=TRUE, nfolds=nfolds, grouped=FALSE )
...
if( opt$valid.data!=0 ){
    ...
    if( !is.null(opt$pred2) ){
        ...
        my_data <- data.frame( target$IID, target[,opt$pheno.name], prs.ridge, prs.ridge1, prs.ridge2, prs.weighted)
        colnames(my_data) <- c('---','pheno','prs','ridge1','ridge2','weighted')                                       
        write.table( my_data, paste0(opt$outfile,"_weighted_combined_prs_predictions.out"), row.names = FALSE, quote=FALSE)
...
for( chr in 1:22 ){
    beta.bar1 <- fread(paste0(opt$eval1,"_beta_bar_chr",chr,".txt.gz"),data.table=FALSE)
    beta.bar1 <- beta.list(beta.bar1)

    if( !is.null(opt$eval2) ){
        beta.bar2 <- fread(paste0(opt$eval2,"_beta_bar_chr",chr,".txt.gz"),data.table=FALSE)
        beta.bar2 <- beta.list(beta.bar2)

        kl <- fread(paste0(opt$eval2,"_KLdist_chr",chr,".txt.gz"),data.table=FALSE)[,-1]
        if( chr==1 ){
