...
w.ridge1 <- as.vector(coef( fit.ridge1, as.matrix(pred1), s='lambda.min' )[-1])
...
#    w.ridge2 <- getGlmnetFit( fit.ridge2, as.matrix(pred2), s='lambda.min', sparse=FALSE )[-1]
    w.ridge <- as.vector(coef( fit.ridge, X, s='lambda.min' )[-1])
    w.ridge2 <- as.vector(coef( fit.ridge2, as.matrix(pred2), s='lambda.min' )[-1])
