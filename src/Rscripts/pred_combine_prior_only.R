...
for( chr in 1:22 ){
    ...
    beta.bar2 <- fread(paste0(opt$models1,"_beta_bar_chr",chr,".txt.gz"),data.table=FALSE)
    beta.bar2 <- beta.list(beta.bar2)
    kl <- fread(paste0(opt$models1,"_KLdist_chr",chr,".txt.gz"),
                data.table=FALSE)[,-1]
    if( chr==1 ){
    ...
    beta.bar.genome <- rbind( beta.bar.genome, beta.bar22 )
}
