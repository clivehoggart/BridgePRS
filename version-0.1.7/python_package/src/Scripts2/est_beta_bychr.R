...
option_list = list(
    ...
    make_option(c("-c", "--clump.stem"), type="character",help="Clump stem", metavar="character"),
    make_option(c("-s", "--sumstats"), type="character",help="Sum stats file", metavar="character"),
    make_option(c("--sumstats2"), type="character",help="Sum stats file pop2", metavar="character", default=NULL),
    make_option(c("--thinned.snplist"), type="character",help="List of SNPs to thin to", metavar="character", default=0),
    make_option(c("--n.max.locus"), type="numeric",help="Large loci are thinned", metavar="numeric", default=0),
    make_option(c("--ld.ids"), type="character",help="File listing IDs to use for estimating LD", metavar="character"),
    make_option(c("--param.file"), type="character", default=NULL,help="File of lambda and S parameters", metavar="character"),
    make_option(c("-b", "--beta.stem"), type="character", default=NULL,help="Out file stem", metavar="character"),
    make_option(c("--lambda","-l"), type="character", default="1",help="Comma delimited lambda values", metavar="character"),
    make_option(c("--S"), type="character", default="1",help="Comma delimited S values", metavar="character"),
    make_option(c("--precision"), type="logical", default=FALSE,help="Option to calculate precision matrix of each clump",metavar="character"),
    make_option(c("--n.cores"), type="numeric", default=1,help="Number of processors for mclapply to use", metavar="character"),
    make_option(c("--bfile"), type="character",help="Plink file with test & train2 genotype data", metavar="character"),
    make_option(c("--sumstats.snpID"), type="character", default="SNP",help="SNP column name", metavar="character"),
    make_option(c("--sumstats.betaID"), type="character", default="BETA",help="Beta column name", metavar="character"),
    make_option(c("--sumstats.allele0ID"), type="character", default="ALLELE0",help="Allele 0 column name", metavar="character"),
    make_option(c("--sumstats.allele1ID"), type="character", default="ALLELE1",help="Allele 1 column name", metavar="character"),
    make_option(c("--sumstats.nID"), type="character", default="OBS",help="N. obs column name", metavar="character"),
    make_option(c("--sumstats.seID"), type="character", default="SE",help="SE column name", metavar="character"),
    make_option(c("--sumstats.frqID"), type="character", default="FRQ",help="Freq column name", metavar="character"),
    make_option(c("--strand.check"), type="numeric", default=0,help="Keep only non-ambiguous SNPs", metavar="numeric"),
    #make_option(c("--ld.shrink"), type="numeric",help="Indicator for shrinking LD matrix", metavar="numeric", default=0),
    #make_option(c("--recomb.file"), type="character", default=NULL,help="File of genome-wide recombination rates", metavar="character"),
    #make_option(c("--Ne"), type="numeric",help="Effective population size", metavar="numeric", default=0),
    make_option(c("--by.chr"), type="numeric", default=1,help="Logical indicating if bed files are split by chr",metavar="character"),
    ...
    fits <- mclapply( 1:length(clump.use),
                     function(i){
                         read.fit.clump( clump.i=clump[clump.use[i],],
                                        ...
                                        do.ld.shrink=0,
                                        sumstats=sumstats,#recomb=recomb, Ne=opt$Ne,
