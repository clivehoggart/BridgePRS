...
RSCRIPTS=../python_package/src/Scripts
...
then
    ...
    #Rscript --vanilla ~/BridgePRS1/src/pred_combine_en.R \
    Rscript --vanilla $RSCRIPTS"/"pred_combine_en.R \
	    --fpath $FPATH \
            --pred1 $outdir/${pop2}_stage1 --pred2 $outdir/${pop2}_stage2 \
